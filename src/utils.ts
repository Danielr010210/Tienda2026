 ...
 * Uses the native browser Web Crypto API when available, and falls back to a pure-JS implementation
 * when executed within sandboxed or cross-origin iframes where `crypto.subtle` is blocked.
 */
export async function hashSHA256(text: string): Promise<string> {
  try {
    ...
    console.warn('Web Crypto API blocked or failed, falling back to pure-JS SHA-256:', e);
  }

  // Fallback: Pure-JS implementation of SHA-256 (compatible with UTF-8 characters)
  ...
  const words: number[] = [];
  const asciiLength = utf8Text.length;
  ...
  for (i = 0; i < asciiLength; i++) {
    words[i >> 2] |= (utf8Text.charCodeAt(i) & 0xff) << (24 - (i % 4) * 8);
  }
